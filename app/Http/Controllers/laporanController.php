...
class laporanController extends Controller
{
    ...
    public function kinerjaByOpd(){
        ...
        foreach ($pegawai as $key => $value) {
            $aktivitas = DB::table('tb_aktivitas')->select('id','id_pegawai','hasil',DB::raw("SUM(waktu) as count"))->whereMonth('tanggal',$bulan)->where('id_pegawai',$value->id)->where('kesesuain','0')->whereNotNull('id_pegawai')->get();           
